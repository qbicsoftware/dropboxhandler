...
def listen(listendir, maxqueue=0, task_re=None, interval=2, stop_event=None,
           beat_interval=30):
    ...
    while True:
        if stop_event.is_set():
            logging.info("Got shutdown event")
            break
        ...
        for dir in dirs:
            ...
            if (dir / "START").exists():
                ...
                try:
                    task = FSRequest(dir, beat_interval=beat_interval)
...
class FSRequest(object):
    ...
    def __init__(self, dir, uuid=None, beat_interval=5):
    ...
    def beat(self):
        try:
            self._start_beat(interval=self._beat_interval)
            yield
        finally:
            self._stop_beat()
    ...
    def _start_beat(self, interval):
        self._stop_beating_flag = threading.Event()

        def beat():
            while not self._stop_beating_flag.is_set():
                self._single_beat()
                time.sleep(interval)
...
def submit(serverdir, infiles, beat_timeout=None):
    ...
    future = FSFuture(serverdir, infiles, beat_timeout=beat_timeout)
    future._start()
    return future
...
class FSFuture:
    ...
    """
    def __init__(self, serverdir, infiles, beat_timeout=30, timeout=30):
        try:
